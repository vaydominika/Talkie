...
export function JapaneseReview({ reviewAttempts }: { reviewAttempts: ReviewAttempt[] }) {
  ...
  const vocabularyMisses = reviewAttempts.filter((attempt) => !attempt.correct);
  ...
  const vocabularyCounts = useMemo(() => {
    ...
    vocabularyMisses.forEach((attempt) => counts.set(attempt.vocabularyEntryId, (counts.get(attempt.vocabularyEntryId) ?? 0) + 1));
    return counts;
  }, [vocabularyMisses]);
  ...
  return (
    <div className="space-y-6">
      ...
      <ReviewSection
        ...
        title="Vocabulary misses"
        correct={reviewAttempts.filter((attempt) => attempt.correct).length}
        missed={vocabularyMisses.length}
      >
        {vocabularyMisses.length ? (
              ...
              <tbody>
                {vocabularyMisses.map((attempt) => (
                  <tr key={attempt.id} className="border-b last:border-0">
                    ...
                    <td className="py-3 font-semibold text-rose-700">{vocabularyCounts.get(attempt.vocabularyEntryId)}</td>
          ...
          <p className="mt-5 text-sm text-muted-foreground">No wrong vocabulary flashcard answers yet.</p>
        )}
      </ReviewSection>
    </div>
  );
...
function ReviewSection({ eyebrow, title, correct, missed, children }: { eyebrow: string; title: string; correct: number; missed: number; children: React.ReactNode }) {
  return (
    <section className="animate-panel-in rounded-lg border p-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="text-right text-sm">
          ...
          <p className="font-semibold text-rose-700">{missed} missed</p>
        </div>
      </div>
      {children}
    </section>
