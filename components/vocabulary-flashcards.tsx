...
import { saveVocabularyAttempt } from "@/lib/vocabulary-review";
...
export function VocabularyFlashcards({ words, selectedIds }: { words: Word[]; selectedIds: Set<string> }) {
  ...
  useEffect(() => {
    setDeck(buildDeck(selectedWords, direction, randomOrder));
    setIndex(0);
    setAnswer("");
    setStatus("idle");
    setRevealed(false);
  }, [selectedWords, direction, randomOrder]);
  ...
  const resetDeck = (nextDirection = direction, nextRandomOrder = randomOrder) => {
  ...
  if (!selectedWords.length) {
    return (
      <section className="rounded-2xl border border-dashed p-8 text-center">
        <h2 className="font-serif text-2xl">No vocabulary flashcards yet.</h2>
  ...
  return (
    <section className="mx-auto max-w-xl rounded-[1.75rem] border bg-[#fbfaf4] p-6 text-center shadow-sm dark:bg-stone-950">
      <p className="font-mono text-xs uppercase tracking-[0.18em] text-rose-700">Vocabulary sprint</p>

      <div className="mt-5 flex flex-wrap justify-center gap-2">
        <Toggle active={direction === "target-native"} onClick={() => resetDeck("target-native")}>
          Word → English
        </Toggle>
        <Toggle active={direction === "native-target"} onClick={() => resetDeck("native-target")}>
          English → Word
        </Toggle>
        <Toggle active={direction === "random"} onClick={() => resetDeck("random")}>
          Random side
        </Toggle>
        <Toggle active={randomOrder} onClick={() => resetDeck(direction, !randomOrder)}>
